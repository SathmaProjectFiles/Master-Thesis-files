# -*- coding: utf-8 -*-
"""
Created on Mon May  6 22:41:57 2024
...
from ConsumBaselineV1 import CreateConsumBaselineDf
...
# Load:
    ...
    #3. Maximum consumption (P_max),
    #4. FCRD-Up capacity reserves (Up_consum),
    #5. FCRD-Down capacity reserves (Down_consum)   
# for all hours from Monday to Sunday: 
consum_Baseline, P_min_consum, P_max, Up_consum, Down_consum = CreateConsumBaselineDf() 
...
for n in range(7): # To access n-th day of the week
    ...
    c_B.columns =['Consumption'] # Change the column name in the dataframe: 'c_B'
    P_min = P_min_consum[n] # Extract minimum consumption values for the n-th day of the week
    P_min.columns =['P_min'] # Change the column name in the dataframe: 'P_min'
    ...
    Up_cap.columns = ['Up reserve'] # Change the column name in the dataframe: 'Up_cap'
    Down_cap = Down_consum[n] # Extract FCRD-Down reserves for the n-th day of the week
    Down_cap.columns = ['Down reserve'] # Change the column name in the dataframe: 'Down_cap'
...
for d in range(7):
    ...
    for h in range(24):
        ...
        priceD_day = Prices_FCRDwn[d]
        priceD_day.columns = ['Price - D']
        
        cU_day = cU_list[d]
        cU_day.columns = ['c_U values']
        priceU_day = Prices_FCRUp[d]
...
plt.figure(figsize=(10, 6), dpi=80)
for d in range(7):
    plt.plot(merged_FCRDwnIncome[d])
plt.legend( ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"], loc = "upper right")
plt.xlabel("Hour of day")
